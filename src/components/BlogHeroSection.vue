<template>
    <div class="main-service-hero-section">
        <div class="hero-details">
            <div class="service-hero-texting">
                <h1 class="hero-text">News and &nbsp;<span class="animated">Articles.</span></h1>
            </div>
            <div class="hero-stars">
                <!-- Dots randomly positioned -->
                <div class="star" style="top: 10%; left: 20%;"></div>
                <div class="star" style="top: 30%; left: 80%;"></div>
                <div class="star" style="top: 50%; left: 50%;"></div>
                <div class="star" style="top: 70%; left: 10%;"></div>
                <div class="star" style="top: 95%; left: 40%;"></div>
                <div class="star" style="top: 85%; left: 60%;"></div>
                <div class="star" style="top: 135%; left: 88%;"></div>
                <div class="hero-paragraph">
                    <p>Explore our latest insights and stories, where creativity meets innovation. From
                        behind-the-scenes looks at our 3D animation projects to industry trends and tips, our blog is
                        your go-to source for inspiration and expertise.</p>
                </div>
            </div>
        </div>
    </div>
</template>